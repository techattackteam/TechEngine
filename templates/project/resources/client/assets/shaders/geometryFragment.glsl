...
void main() {
    ...
    // IBL generation is currently disabled, so its textures are not valid inputs.
    // Preserve the sampled material albedo until the skybox/IBL pass is restored.
    //vec3 ambient = (diffuse + specular) * ao;
    vec3 ambient = material.albedo.rgb;
