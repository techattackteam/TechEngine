...
namespace TechEngine {
    ...
    Scene& EntityHierarchyPanel::getActiveScene() {
        return m_appSystemsRegistry.getSystem<ScenesManager>().getActiveScene();
    }
    ...
    void EntityHierarchyPanel::rebuildCache() {
        ...
        m_totalNodeCount = 0;
        scene.runSystem<Entity, Tag>([this, &scene, &oldExpansionStates, &currentRoots](Entity entity, Tag& tag) {
            ++m_totalNodeCount;
        ...
        // Rebuild root list preserving old order where possible
        // First, add entities that were in the old order and still exist as roots
        ...
        // Then add any new root entities that weren't in the old order
    ...
    std::string EntityHierarchyPanel::getNodeName(Entity id) const {
        ScenesManager& scenesManager = const_cast<EntityHierarchyPanel*>(this)->m_appSystemsRegistry.getSystem<ScenesManager>();
        Scene& scene = scenesManager.getActiveScene();
    ...
    void EntityHierarchyPanel::createEntity(const EntityToCreate& entityInfo) {
        ...
        switch (entityInfo.type) {
            ...
            case EntityType::Cube:
                ...
                scene.addComponent<MeshRenderer>(entityCreated,
                    ComponentsFactory::createMeshRenderer(
                        &resourcesManager.getDefaultMesh(),
                        &resourcesManager.getDefaultMaterial()));
                break;
            ...
            case EntityType::Sphere:
                ...
                scene.addComponent<MeshRenderer>(entityCreated,
                    ComponentsFactory::createMeshRenderer(
                        &resourcesManager.getMesh("Sphere"),
                        &resourcesManager.getDefaultMaterial()));
                break;
