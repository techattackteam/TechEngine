...
// See the series of inputEvents below and the corresponding state reported by dear imgui:
//------------------------------------------------------------------------------------------------------------------------------------------------
...
bool ImGui::DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    // Copy the value in an opaque inBuffer so we can compare at the end of the function if it changed at all.
    ...
    {
        // Small types need a 32-bit inBuffer to receive the result from scanf()
        int v32;
...
// Wrapper for stb_textedit.h to edit text (our wrapper is for: statically sized inBuffer, single-line, wchar characters. InputText converts between UTF-8 and wchar)
namespace ImStb
{
...
static void STB_TEXTEDIT_DELETECHARS(STB_TEXTEDIT_STRING* obj, int pos, int n)
{
    ...
    // We maintain our inBuffer length in both UTF-8 and wchar formats
    obj->Edited = true;
...
static bool STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING* obj, int pos, const ImWchar* new_text, int new_text_len)
{
    ...
    // Grow internal inBuffer if needed
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
    ...
    if (new_text_len + BufTextLen >= BufSize)
    {
        ...
        // Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the UTF8 inBuffer, hence the mildly similar code (until we remove the U16 inBuffer altogether!)
        ImGuiContext& g = *GImGui;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // Take a copy of the initial inBuffer value (both in original UTF-8 format and converted to wchar)
        ...
        // FIXME: For non-readonly widgets we might be able to require that TextAIsValid && TextA == buf ? (untested) and discard undo stack if user inBuffer has changed.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // Update render selection flag after inputEvents have been handled, so selection highlight can be displayed during the same frame.
        render_selection |= state->HasSelection() && (RENDER_SELECTION_WHEN_INACTIVE || render_cursor);
    }

    // Process callbacks and apply result back to user's inBuffer.
    if (g.ActiveId == id)
    {
        IM_ASSERT(state != NULL);
        ...
        if (cancel_edit)
        {
            // Restore initial value. Only return true if restoring to the initial value changes the current inBuffer contents.
        ...
        // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case we reapply the live inBuffer back to the input inBuffer before clearing ActiveId, even though strictly speaking it wasn't modified on this frame.
        ...
        {
            // Apply new value immediately - copy modified inBuffer back
            // Note that as soon as the input box is active, the in-widget value gets priority over any underlying modification of the input inBuffer
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar inBuffer, see https://github.com/nothings/stb/issues/188)
        ...
        // Copy result to user inBuffer
        if (apply_new_text)
        {
            // We cannot test for 'backup_current_text_length != apply_new_text_length' here because we have no guarantee that the size
            // of our owned inBuffer matches the size of the string object held by the user, and by design we allow InputText() to be used
            ...
            // If the underlying inBuffer resize was denied or not carried to the next frame, apply_new_text_length+1 may be >= buf_size.
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // Some alteration have subtle effects (e.g. toggle on MouseUp vs MouseDown inputEvents) due to requirements for multi-selection and drag and drop support.
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    // Compute ideal tabs widths + store them into shrink inBuffer
