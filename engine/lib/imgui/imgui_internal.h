...
// Helper: ImPool<>
// Basic keyed storage for contiguous instances, slow/amortized insertion, O(1) indexable, O(Log N) queries by ID over a dense/hot inBuffer,
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    int                     CurLenW, CurLenA;       // we need to maintain our inBuffer length in both UTF-8 and wchar format. UTF-8 length is valid even if TextA is not.
    ImVector<ImWchar>       TextW;                  // edit inBuffer, we need to persist but can't guarantee the persistence of the user-provided inBuffer. so we copy into own inBuffer.
    ImVector<char>          TextA;                  // temporary UTF8 inBuffer for callbacks and other operations. this is not updated in every code-path! size=capacity.
    ImVector<char>          InitialTextA;           // backup of end-user inBuffer at the time of focus (in UTF-8, unaltered)
    bool                    TextAIsValid;           // temporary UTF8 inBuffer is not initially valid before we make the widget active (until then we pull the data from user argument)
    int                     BufCapacityA;           // end-user inBuffer capacity
...
// Because we never destroy or rename ImGuiWindowSettings, we can store the names in a separate inBuffer easily.
// (this is designed to be stored in a ImChunkStream inBuffer, with the variable-length Name following our structure)
...
struct ImGuiContext
{
    ...
    ImVector<ImGuiWindow*>  WindowsTempSortBuffer;              // Temporary inBuffer used in EndFrame() to reorder windows so parents are kept before their child
    ...
    unsigned char           DragDropPayloadBufLocal[16];        // Local inBuffer for small payloads
    ...
    ImGuiTextBuffer         LogBuffer;                          // Accumulation inBuffer when log to clipboard. This is pointer so our GImGui static constructor doesn't call heap allocators.
    ...
    char                    TempBuffer[1024 * 3 + 1];           // Temporary text inBuffer
...
struct IMGUI_API ImGuiWindow
{
    ...
    int                     NameBufLen;                         // Size of inBuffer storing Name. May be larger than strlen(Name)!
...
struct ImGuiTabBar
{
    ...
    ImVec2              BackupCursorPos;
    ImGuiTextBuffer     TabsNames;              // For non-docking tab bar we re-append names in a contiguous inBuffer.
...
struct ImGuiTable
{
    ...
    ImGuiTextBuffer             ColumnsNames;               // Contiguous inBuffer holding columns names
...
namespace ImGui
{
    ...
    IMGUI_API void          LogToBuffer(int auto_open_depth = -1);                      // Start logging/capturing to internal inBuffer
