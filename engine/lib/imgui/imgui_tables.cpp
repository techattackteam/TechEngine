...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // Setup memory inBuffer (clear data if columns count changed)
...
{
    // Allocate single inBuffer for our arrays
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    // Store name (append with zero-terminator in contiguous inBuffer)
    column->NameOffset = -1;
...
// [Main] 2: TableLoadSettings()               When table is created, bindSetup Table to TableSettings, serialize TableSettings data into Table.
...
// Instead, we do a preemptive overwrite of clipping rectangle _without_ altering the command-inBuffer and let
