...
namespace TechEngine {
    ...
    void ExportSettingsPanel::onUpdate() {
        ...
        if (ImGui::Button("Export Game")) {
            editorRegistry.getSystem<ProjectManager>().exportProject(ProjectType::Client, CompileMode::RELEASE);
        }
        if (ImGui::Button("Export Server")) {
            editorRegistry.getSystem<ProjectManager>().exportProject(ProjectType::Server, CompileMode::RELEASE);
        }
    ...
    void ExportSettingsPanel::exportServerProject(CompileMode compileMode) {
        ...
        try {
            ...
            if (compileMode == RELEASE) { // Happens when building the game to build path. If in debug engines is in debug mode
                ...
                exportPath/* = projectManager.getProjectServerExportPath()#2#;
