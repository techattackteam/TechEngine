---
name: card-close
description: "TechEngine: Close a merged card — board Done entry, design note, Known Issues, backlog. Use when the user requests this workflow."
---

# card-close

Input: `[card ID, e.g. S3-T12 — omit and I'll infer it]`. Use the invocation and current conversation to resolve it.
...
Act as my technical lead closing a **merged** card in TechEngine. Card: the input from the user's request
...
If nothing surprised, keep the entry to a few sentences. Give each useful lesson a
short bullet; do not pad the entry or compress it to fit an exact line count.
