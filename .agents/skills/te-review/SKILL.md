---
name: te-review
description: "TechEngine: Review code against CONVENTIONS.md's judgment rows and the ADR structural invariants. Use when the user requests this workflow."
---

# te-review

Input: `[file, folder, or module — omit and I'll take the working tree diff]`. Use the invocation and current conversation to resolve it.
...
Act as my reviewer for TechEngine house rules. Target: the input from the user's request
...
`CONVENTIONS.md` → *Mechanical vs judgment* splits enforcement three ways, and this skill is
...
**Not a bug hunt either.** Request a separate correctness review for logic errors and edge cases. If you
...
Use one short bullet per finding, with `file:line`, the rule, and the correction in
complete sentences. Use **Violations**, **Open row fired**, and **Off-rubric** only
where they contain findings. Do not compress explanations into chains of symbols.
