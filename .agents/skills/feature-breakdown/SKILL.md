...
description: "TechEngine: Decompose a feature into Epic → Story → Task, session-sized. Use when the user requests this workflow."
---

# feature-breakdown

Input: `<feature>`. Use the invocation and current conversation to resolve it.
...
equal driver** (draft and challenge; I decide scope): the input from the user's request

Keep each task to one readable bullet with its title, done-condition, and tags.
Let it wrap naturally. Keep light design notes short; put rationale in the artifact
...
3. **Artifact gate** — run each item through
   ...
   (short note, drafted at weekend planning) or **heavy** (ADR / needs a spike → its own
