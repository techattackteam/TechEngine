---
name: sprint-plan
description: "TechEngine: Plan the next 2-week sprint (boundary-weekend ritual). Use when the user requests this workflow."
---

# sprint-plan

Input: `[sprint theme or focus, if you have one in mind]`. Use the invocation and current conversation to resolve it.
...
**This skill absorbs `$weekly-review` on a boundary weekend — do not run both** (decided
...
My input on focus (may be empty): the input from the user's request

Use the templates as prompts. Keep the retro focused on lessons and the sprint note
focused on the goal and work. Write one readable bullet per task with its title,
done-condition, and tags; let it wrap naturally. Link existing records instead of
recapping them. Do not fill sections merely to make the note look complete.
