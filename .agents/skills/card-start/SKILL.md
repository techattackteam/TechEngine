---
name: card-start
description: "TechEngine: Ground a card before building it — freshness check, design note, open defects, branch. Use when the user requests this workflow."
---

# card-start

Input: `[card ID, e.g. S3-T14 — omit and I'll take the next one on the board]`. Use the invocation and current conversation to resolve it.
...
2. **Freshness, first, because it decides how much to trust everything after it.**
   ...
   `git fetch origin && git log --oneline <sha>..origin/master`
...
If the ground is clean, say so briefly with the acceptance and verification limits.
Otherwise, explain each mismatch in a short paragraph or bullet. Include only what
changes how I build the card; there is no fixed line count to fill.

## Then

If I already asked you to create the branch, create it. Otherwise, offer to cut it:

`git fetch origin && git switch -c <card ID>/<slug> origin/master`
