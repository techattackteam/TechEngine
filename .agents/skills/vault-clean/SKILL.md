---
name: vault-clean
description: "TechEngine: Sweep the vault for stale, redundant, malformed, or orphaned content and clean it. Use when the user requests this workflow."
---

# vault-clean

Input: `[optional folder to scope to, e.g. '03 Architecture']`. Use the invocation and current conversation to resolve it.
...
folder if given: the input from the user's request) and leave it **current, lean, and well-formed** — without
losing anything load-bearing. Scope is the `docs/` vault only; the AI memory files are
outside this vault workflow.
...
the content being flagged or compress explanations to force one-line entries.
...
2. **Malformed** — empty template fields (`| **X** | |`, `- Blocker:` with nothing),
   unfilled placeholders (`YYYY-MM-DD`, `<title>`, `_none logged_`, stray `the input from the user's request`),
   headings with no body, broken tables, stray characters (e.g. a leading `/#`),
   **ASCII diagrams that should be Mermaid** (AGENTS.md), a sentence that lost its link.
3. **Redundant** — the same fact in more than one note, or content restating `AGENTS.md`,
...
6. **Bloat** — sections over ~30 lines, notes covering more than one topic, append-only
   growth, or prose where a table/bullets would be clearer. File length alone is not a
   reason to split; a long single-topic design note is allowed (AGENTS.md).
...
2. **Auto-apply only the safe mechanical fixes** — formatting, dead empty fields, typos,
   ...
   fix affects.
3. **Keep lossy changes within the user's explicit authorization** — deleting a note,
   merging notes, rewriting content, removing a section, or splitting a note. If not
   already authorized, finish the mechanical fixes, prepare the proposed change, then ask.
